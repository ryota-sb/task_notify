        ...
        <v-col>
          <v-card>
            <v-toolbar
              ...
              flat
            >
              <v-toolbar-title class="body-2">タスク追加</v-toolbar-title>
            </v-toolbar>
            <v-card-text>
              <v-text-field
                v-model="newTask"
              ...
              <v-btn text icon color="light-green lighten-1" @click="createTask()">
                <v-icon color="green lighten-1">create</v-icon>
              </v-btn>
            </v-card-text>
          </v-card>
        </v-col>
...
export default {
  ...
  methods: {
    ...
    createTask() {
      if (this.newTask == '' || this.newTask.length > 40) {
        this.success_state = false;
        this.error_state = true;
        return 
      }
      axios.post('/api/v1/tasks', { task: { content: this.newTask } }).then((response) => {
