...
export default {
  ...
  methods: {
    createTask(newTask, time) {
      ...
      this.$store.dispatch('createTaskAction', { newTask: newTask, week: 'sunday', time: time, user_id: user_id })
    },
    updateTask(task_id) {
    ...
    doneTasks() {
      ...
      return task.filter(task => !task.is_done && task.week == "sunday" && task.user_id == user_id)
    },
    notDoneTasks() {
      let task = this.$store.state.tasks
      let user_id = this.$store.state.users.data.id
