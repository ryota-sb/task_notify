        ...
        <v-col>
          <v-card>
            <v-toolbar
              ...
              flat
            >
              <v-toolbar-title>木曜日のタスク</v-toolbar-title>
            </v-toolbar>
            <v-list
...
export default {
  ...
  methods: {
    createTask(newTask, time) {
      this.$store.dispatch('createTaskAction', { newTask: newTask, week: 'thursday', time: time })
    },
    updateTask(task_id) {
    ...
    doneTasks() {
      ...
      return task.filter(task => !task.is_done && task.week == "thursday")
    },
    notDoneTasks() {
      let task = this.$store.state.tasks
      return task.filter(task => task.is_done && task.week == "thursday")
